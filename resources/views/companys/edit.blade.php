      ...
      <form action="{{route('company.update',['id'=> $company->id])}}" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group">
          ...
          <div class="form-group">
            ...
            <input type="text" name="companyـofficial_name" class="form-control" value="{{$company->companyـofficial_name}}">
          </div>

          <div class="form-group">
            <label for="exampleFormControlInput1">رقم السجل التجاري </label>
            <input type="text" name="commercial_registration_no" class="form-control" value="{{$company->commercial_registration_no}}">
          </div>

          <div class="form-group">
            <label for="exampleFormControlInput1">عقد الشركة </label>
            <input type="file" name="companyـcontract" class="form-control" value="{{$company->companyـcontract}}">
          </div>
          ...
          <div class="form-group d-flex">
            ...
            <input type="date" name="start_decade" class="form-control" style="width: 200px!important;" value="{{$company->start_decade}}">
          </div>

          <div class="form-group d-flex">
            <label for="exampleFormControlInput1">نهاية العقد </label>
            <input type="date" name="end_decade" class="form-control" style="width: 200px!important;" value="{{$company->end_decade}}">
          </div>

          <label for="exampleFormControlInput1">القسم </label>
          <div class="form-group d-flex">
            @foreach ($sections as $section)
            <input type="checkbox" name="sections[]" value="{{$section->id}}" style="margin: 5px;" @foreach ($company->sections as $item)
          ...
          <div class="form-group">
            <label class="switch">
              <input name="status" type="checkbox" {{$company->status == 1 ? 'checked' : '' }}>
